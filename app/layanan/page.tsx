    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-8">
      ...
      <div className="space-y-4">
        {layananData.map((item) => (
          <Card key={item.id}>
            <h2 className="font-semibold text-slate-900">{item.nama}</h2>
            <p className="mt-1 text-sm text-slate-700">{item.deskripsi}</p>

            <h3 className="mt-3 text-sm font-semibold text-slate-800">Syarat</h3>
            ...
            <p className="mt-3 text-sm text-slate-600">
              Hubungi:{" "}
              <Link href="/kontak" className="text-emerald-700 underline">
                {item.kontakJabatan}
              </Link>
            </p>
          </Card>

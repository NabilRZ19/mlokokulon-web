...
export function EditUserForm({
  ...
  // Hanya Tier 1 yang boleh mengedit akun Tier 1 (termasuk akun ini sendiri)
  const isLocked = actorTier !== 1 && targetTier === 1;
  ...
  if (isLocked) {
    return (
      <div>
        ...
        <div className="mx-auto max-w-xl rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
          Akun ini adalah Super Admin (Tier 1). Hanya sesama Super Admin yang bisa mengedit akun Tier 1.
        </div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="mx-auto max-w-xl space-y-6">
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm space-y-5">
          ...
          <div>
            ...
            {actorTier !== 1 && (
              <p className="mt-1 text-[11px] text-muted-foreground">
                Hanya Super Admin (Tier 1) yang bisa menaikkan akun jadi Tier 1.
              </p>
            )}
          </div>
        </div>

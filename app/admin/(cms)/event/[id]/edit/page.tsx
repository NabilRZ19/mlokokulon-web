  ...
  useEffect(() => {
    async function initData() {
      ...
      try {
        ...
        if (!dataRes.ok) throw new Error("Gagal mengambil data event.");
        const data = await dataRes.json();

        setJudul(data.judul || "");
        setSlug(data.slug || "");
        setDeskripsi(data.deskripsi || "");
        setTanggalMulai(data.tanggal_mulai ? String(data.tanggal_mulai).split("T")[0] : new Date().toISOString().split("T")[0]);
        setTanggalSelesai(data.tanggal_selesai ? String(data.tanggal_selesai).split("T")[0] : "");
        setJamMulai(data.jam_mulai || "08:00 WIB");
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <AdminPageHeader
        ...
        description="Perbarui rincian kegiatan atau waktu pelaksanaan."
        actions={
          <Link
            href="/admin/event"

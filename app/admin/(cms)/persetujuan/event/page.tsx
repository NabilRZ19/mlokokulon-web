...
export default function PersetujuanEventPage() {
  ...
  async function fetchPendingList() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/persetujuan/event");
      if (!res.ok) throw new Error("Gagal memuat daftar event pending.");
  ...
  function openModal(item: EventItem, action: "approve" | "reject") {
  ...
  async function handleConfirm(note: string) {
    ...
    setProcessing(true);
    try {
      const res = await fetch(`/api/admin/persetujuan/event/${selectedItem.id}`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Persetujuan Agenda Event"
        actions={
          <>
      ...
      <p className="text-xs text-muted-foreground font-medium">
        Agenda kegiatan yang diajukan oleh Admin RW (Tier 3) atau Admin Kampung KB (Tier 4) dan menunggu persetujuan Anda.
      </p>
      ...
      {loading ? (
        <div className="rounded-xl border border-border bg-card p-8 text-center text-sm text-muted-foreground">
          Memuat daftar pengajuan agenda event…
        </div>
      ) : list.length === 0 ? (
        <div className="rounded-2xl border border-border bg-card p-10 text-center space-y-3 shadow-2xs">
          ...
          <p className="text-base font-bold text-foreground font-heading">Tidak Ada Agenda Event Menunggu Persetujuan</p>
          <p className="text-xs text-muted-foreground max-w-sm mx-auto">Seluruh pengajuan agenda event dari Admin RW (Tier 3) dan Admin Kampung KB (Tier 4) telah diproses.</p>
        </div>
      ) : (
          ...
          {list.map((item) => (
            <div key={item.id} className="rounded-2xl border border-orange-200 bg-orange-50/30 p-4 sm:p-5 shadow-xs space-y-4">
              ...
              <div className="rounded-lg border border-orange-200 bg-white/60 p-3 space-y-1">
                <p className="text-xs font-extrabold uppercase tracking-wider text-orange-700 mb-1.5">Informasi Pengusul &amp; Jadwal</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-foreground">
                  ...
                  <div><span className="text-muted-foreground">Panitia:</span> {item.penulis}</div>
                  <div><span className="text-muted-foreground">Tier Admin:</span> {TIER_LABEL[item.submitted_by_tier ?? 3] ?? "-"}</div>
                  <div><span className="text-muted-foreground">Lokasi:</span> {item.lokasi}</div>
                  <div><span className="text-muted-foreground">Tanggal:</span> {item.tanggal_mulai} {item.tanggal_selesai && item.tanggal_selesai !== item.tanggal_mulai ? `s/d ${item.tanggal_selesai}` : ""}</div>
                  <div><span className="text-muted-foreground">Waktu:</span> {item.jam_mulai}</div>
                </div>
              </div>

              <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{item.deskripsi}</p>

              {/* Actions */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-orange-200">
                <Link
                  href={`/admin/persetujuan/event/${item.id}/preview`}
                  className="inline-flex items-center gap-1 text-xs font-bold text-primary hover:underline"
                >
                  <span>Preview Agenda Event Lengkap</span>

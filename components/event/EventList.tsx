        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredAndSorted.map((item) => {
            ...
            return (
              <article
                key={item.id}
                className="flex flex-col justify-between overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/50 hover:shadow-md group"
              >
                ...
                {/* Card Footer Button */}
                <div className="p-5 pt-3 border-t border-border mt-4">
                  <Link
                    href={`/event/${item.slug}`}
                    className="inline-flex w-full items-center justify-center gap-1.5 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-xs font-bold text-emerald-900 transition-all hover:bg-emerald-600 hover:text-white"
                  >
